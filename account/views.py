...
def dashboard(request):
    ...
    if following_ids:
        #retrieve actions of only those users which the current user is following
...
def edit(request):
    ...
    if request.method == 'POST':
        ...
        if user_form.is_valid() and profile_form.is_valid():
            user_form.save()
            profile_form.save()
            create_action(user,'has edited the profile')
...
def profile_view(request):
    user = request.user
    profile = request.user.profile
    return render(request, 'account/user/profile.html', {'section':'profile', 'user':user, 'profile':profile})

@login_required
def picture_view(request):
    user = request.user
    return render(request, 'account/user/pictures.html', {'section':'pictures', 'user':user})
